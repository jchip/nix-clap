...
const { cbOrVal, dup, makeDefaults, applyDefaults } = require("./xtil");

const SUPPORT_TYPES = ["number", "string", "boolean"];
/*
 * Command
 */
class Command {
    ...
    args.replace(/([<\[])([^>\]]+)([>\]])/g, (a, mark, xname) => {
      ...
      if (name) {
        ...
        if (splits.length > 1) {
          type = splits[0];
          name = splits[1];
          assert(SUPPORT_TYPES.indexOf(type) >= 0, `unknown argument ${a} type ${type}`);
        } else {
          name = splits[0];
        }
      }
