...
const { cbOrVal, camelCase, dup, fitLines, objEach } = require("./xtil");
const SUPPORT_TYPES = ["count", "string", "number", "boolean"];
/*
 * Options
 */
class Options {
      ...
      if (type) {
        ...
        if (type.indexOf("array") >= 0) {
          ...
          if (splits.length > 1) {
            ...
            assert(
              SUPPORT_TYPES.indexOf(opt.subtype) >= 0,
          ...
          assert(
            ...
            `Unknown argument type ${type} for option ${name}`
          );
        }
      }
      opt.name = name;
