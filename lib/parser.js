...
const { CMD, OPTIONS, TARGET } = require("./symbols");

const { camelCase, convertValue } = require("./xtil");
/*
 * Parser
 */
class Parser {
  ...
  convertOptValueType(ctx, value) {
    const { ccLong, opt } = ctx;
    const type = opt.type;

    if (type === "count") {
      value = (ctx[TARGET].opts[ccLong] || 0) + 1;
    } else if (type === "array") {
      if (opt.subtype && Array.isArray(value)) {
        value = value.map(v => convertValue(opt.subtype, v));
      }
    } else {
      value = convertValue(type, Array.isArray(value) ? value[0] : value);
    }
  ...
  setOptValue(name, value) {
    ...
    if (!ctx) {
      this.setUnknownOption(name, value);
      return;
    }
    ...
    if (value !== undefined || opt.type === "count") {
      this.convertOptValueType(ctx, value);
    } else {
      this._states.push(this._state);
  ...
  getOpt(arg) {
    ...
    this.setOptValue(name, value);
  }

  optEndGather() {
    if (this._gathered.length > 0) {
      this.convertOptValueType(this._optCtx, this._gathered);
    } else if (!this._optType || this._optType.indexOf("boolean") >= 0) {
      this._optCtx[TARGET].opts[this._optCtx.ccLong] = true;
    } else {
      assert(!this._optCtx.opt.requireArg, `option ${this._optCtx.name} requires argument`);
      this._optCtx[TARGET].opts[this._optCtx.ccLong] = undefined;
    }
    ...
    const setArg = (name, type, value) => {
      ...
      if (type) {
        ctx.args[name] = Array.isArray(value)
          ? value.map(v => convertValue(type, v))
  ...
  parse(argv, start, parsed) {
    this._argv = argv;
    this._parsed = parsed || { source: {}, commands: [], opts: {} };
